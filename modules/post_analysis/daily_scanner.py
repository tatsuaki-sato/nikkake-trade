...
def run_daily_scanner():
    ...
    # 決算またぎ抑制: 発表直前の銘柄はどれだけスコアが高くても
    # 結果次第で大きくジャンプするイベントギャンブルになるため、新規シグナルを出さない。
    try:
    ...
    for ticker in target_tickers:
        try:
            ...
            if score >= 70: 
                ...
                record_signal(ticker, close_price, target_p, stop_p, score, quant_result['details'], channel="quant_scan")
    ...
    if len(hit_list) > 0:
        ...
        if themes_from_fallback:
            notify_text += "本日は70点以上の高ファクター条件を満たす特定銘柄は検出されませんでした。\n"
            notify_text += "⚠️ 株探ランキングの取得にも失敗したため、以下は市場実勢を反映しない固定の参考リストです(トラッキング登録はスキップ)：\n\n"
        else:
            record_theme_candidates(theme_details, data)
            notify_text += "本日は70点以上の高ファクター条件を満たす特定銘柄は検出されませんでしたが、株探ランキングで現在市場が注目している銘柄グループは以下の通りです：\n\n"
